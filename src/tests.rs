...
#[test]
fn test_spawning_no_collisions() {
    let mut map = create_map();
    setup_map_paths(&mut map);
...
//     let mut agents = create_agents(&map);
//
//     let on_outer = calc_occupied_perim_map(&agents, &map.vehicle_outer_pts, SPAWN_MARGIN, false);
//     let on_inner = calc_occupied_perim_map(&agents, &map.vehicle_inner_pts, SPAWN_MARGIN, true);
//
...
//     // can only be on one of the two lanes/paths
//     assert!(occ_count_inner == 0 || occ_count_outer == 0);
//     assert_eq!(occ_count_outer + occ_count_inner, VEHICLE_LENGTH);
// }
